...
private:
                ...
                bool isTrivial(Instruction const* instruction) const {
                        ...
                        return std::any_of(operands.begin(), operands.end(), [this](auto const& operand) { return isUnit(operand); });
                }

                bool isUnit(Value const* operand) const {
                ...
                void replaceWithArgument(Instruction* instruction) const {
                        ...
                        instruction->replaceAllUsesWith(isUnit(lhs) ? rhs : lhs);
                }
        };
